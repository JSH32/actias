...
local function escapequotes(s) return s:gsub('"', '\\"') end
...
function elementspec.define(name, void)
    local instance = {name = name, void = void}
    instance.__index = instance
    elementspec.specs[name] = setmetatable(instance, element)
    return instance
end

function elementspec.get(name) return elementspec.specs[name] end

element.__index = element

function element.new(name)
    local instance = {children = {}, attrs = {}}
...
function element:opentag()
    ...
    for k, v in pairs(self.attrs) do
        if type(v) == 'string' then v = escapequotes(v) end
        table.insert(t, k .. '=' .. '"' .. v .. '"')
    end
    if #t > 0 then
...
function element:closetag() return '</' .. self.name .. '>' end

function element:appendchild(data) table.insert(self.children, data) end

function element:addattr(attr, value) self.attrs[attr] = value end
...
function element.type(t)
    ...
    while mt do
        if mt == element then return 'element' end
        mt = getmetatable(mt)
    end
end
...
local function defel(name, void)
    ...
    html[name] = function(x) return element.fromvalue(x, name) end
end
...
function html:__call(t) return self.html(t) end
...
defels {
    ...
    'html', -- 4.2 Document metadata
    'head', 'title', {'base', true}, {'link', true}, {'meta', true}, 'style',

    -- 4.3 Sections
    'body', 'article', 'section', 'nav', 'aside', 'h1', 'h2', 'h3', 'h4', 'h5',
    'h6', 'header', 'footer', 'address', -- 4.4 Grouping content
    'p', {'hr', true}, 'pre', 'blockquote', 'ol', 'ul', 'li', 'dl', 'dt', 'dd',
    'figure', 'figcaption', 'div', 'main', -- 4.5 Text-level semantics
    'a', 'em', 'strong', 'small', 's', 'cite', 'q', 'dfn', 'abbr', 'data',
    'time', 'code', 'var', 'samp', 'kbd', 'sub', 'sup', 'i', 'b', 'u', 'mark',
    'ruby', 'rt', 'rtc', 'rp', 'bdi', 'bdo', 'span', {'br', true},
    {'wbr', true}, -- 4.6 Edits
    'ins', 'del', -- 4.7 Embedded content
    {'img', true}, 'iframe', {'embed', true}, 'object', {'param', true},
    'video', 'audio', {'source', true}, {'track', true}, 'map', {'area', true},

    -- 4.9 Tabular data
    'table', 'caption', 'colgroup', {'col', true}, 'tbody', 'thead', 'tfoot',
    'tr', 'td', 'th', -- 4.10 Forms
    'form', 'label', {'input', true}, 'button', 'select', 'datalist',
    'optgroup', 'option', 'textarea', {'keygen', true}, 'output', 'progress',
    'meter', 'fieldset', 'legend', -- 4.11 Scripting
