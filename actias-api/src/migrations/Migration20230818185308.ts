...
export class Migration20230818185308 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table "resources" ("id" uuid not null default gen_random_uuid(), "created_at" timestamptz(0) not null default now(), "updated_at" timestamptz(0) not null default now(), "resource_type" text check ("resource_type" in (\'script\')) not null, "service_id" varchar(255) not null, "project_id" uuid not null, constraint "resources_pkey" primary key ("id"));');

    this.addSql('create table "access" ("id" uuid not null default gen_random_uuid(), "created_at" timestamptz(0) not null default now(), "updated_at" timestamptz(0) not null default now(), "user_id" uuid not null, "project_id" uuid not null, "permission_bitfield" int not null, constraint "access_pkey" primary key ("id"));');
    ...
    this.addSql('alter table "resources" add constraint "resources_project_id_foreign" foreign key ("project_id") references "projects" ("id") on update cascade;');
