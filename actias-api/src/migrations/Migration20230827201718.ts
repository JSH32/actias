...
export class Migration20230827201718 extends Migration {
  async up(): Promise<void> {
    ...
    this.addSql(
      'create table "access" ("id" uuid not null default gen_random_uuid(), "created_at" timestamptz(0) not null default now(), "updated_at" timestamptz(0) not null default now(), "user_id" uuid not null, "project_id" uuid not null, "permission_bitfield" varchar(255) not null, constraint "access_pkey" primary key ("id"));',
    );
    this.addSql(
    ...
    this.addSql(
      'alter table "user_auth_method" add constraint "user_auth_method_user_id_foreign" foreign key ("user_id") references "users" ("id") on update cascade on delete cascade;',
    );

    this.addSql(
      'alter table "projects" add constraint "projects_owner_id_foreign" foreign key ("owner_id") references "users" ("id") on update cascade on delete cascade;',
    );

    this.addSql(
      'alter table "access" add constraint "access_user_id_foreign" foreign key ("user_id") references "users" ("id") on update cascade on delete cascade;',
    );
    this.addSql(
      'alter table "access" add constraint "access_project_id_foreign" foreign key ("project_id") references "projects" ("id") on update cascade on delete cascade;',
    );
  }
}
