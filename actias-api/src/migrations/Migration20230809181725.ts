...
export class Migration20230809181725 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table "access" ("id" uuid not null default gen_random_uuid(), "created_at" timestamptz(0) not null default now(), "updated_at" timestamptz(0) not null default now(), "user_id" uuid null, "project_id" uuid null, "permission_bitfield" int not null, constraint "access_pkey" primary key ("id"));');
    this.addSql('alter table "access" add constraint "access_user_id_project_id_unique" unique ("user_id", "project_id");');

    this.addSql('alter table "resources" add constraint "resources_project_id_foreign" foreign key ("project_id") references "projects" ("id") on delete cascade;');

    this.addSql('alter table "user_auth_method" add constraint "user_auth_method_user_id_foreign" foreign key ("user_id") references "users" ("id") on delete cascade;');

    this.addSql('alter table "access" add constraint "access_user_id_foreign" foreign key ("user_id") references "users" ("id") on delete cascade;');
    this.addSql('alter table "access" add constraint "access_project_id_foreign" foreign key ("project_id") references "projects" ("id") on delete cascade;');
  }
