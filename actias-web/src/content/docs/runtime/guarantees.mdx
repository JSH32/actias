...
<table>
  ...
  <tbody>
    ...
    <tr><td>Any reply from an object</td><td>waits while the object has writes still in flight, whoever wrote them; a settled object answers at once</td><td>the same unknown outcome as a write, after the durability gate's ten seconds</td></tr>
    ...
    <tr><td>Requests per second</td><td>unbounded by default; a project policy sets a rate, burst of the same size</td><td>429 with <code>Retry-After</code>; the refusal costs no vm</td></tr>
    <tr><td>Work units per second</td><td>unbounded by default; a project policy sets a rate, charged after each call</td><td>the project is in debt and refused until it refills</td></tr>
    <tr><td>A node's pools</td><td>requests, blocking work, connections, resident objects, directory queries, shipping lanes, each split fairly among the projects using the node with a floor of 5%</td><td>over its share a project is refused with 429; under contention the floor is what it keeps</td></tr>
    <tr><td>Egress hosts</td><td>a project's deny list, then its allow list (empty admits everything not denied), then the node's own policy: private and local addresses are refused unless the operator allows them</td><td>the request or dial is refused, naming the rule</td></tr>
    <tr><td>Cross-region call</td><td>one hop to the object's region</td><td>one region's round trip added; a project being moved answers "moving", retryably</td></tr>
  </tbody>
</table>
...
the blob store when there are none. Nor is any other reply from that
object while those writes are still in flight: a read, a method that
wrote nothing, a database read served from the object's own file, all
wait the same few milliseconds, so nothing outside the object ever
acts on state a crash could take back. Anything you were told
succeeded survives the loss of the node that ran it, and the next
node to hold the object takes it over from a replica without touching
the store.
...
<table>
  ...
  <tbody>
    ...
    <tr><td>a region becomes unreachable</td><td>everything the region's bucket had replicated elsewhere; the operator rehomes the project and the next touch restores it there</td><td>writes inside the replication lag, stated as such; a region that returns cannot write, because an object's home is a fence on every flight</td></tr>
  </tbody>
</table>
