...
flowchart LR
    ...
    ticket --> flight[ship flight writes the manifest]
    flight --> answer([answer released])
    flight -. 10 s without a manifest .-> unknown([outcome unknown])
```
...
<table>
  ...
  <tbody>
    ...
    <tr><td>Durability gate</td><td>10 seconds for a write to reach storage</td><td>the call fails with an unknown outcome; the write keeps trying to ship</td></tr>
    ...
    <tr><td>Database replica age</td><td>30 seconds for <code>read</code> and <code>read_one</code></td><td>the next read restores a fresher copy</td></tr>
...
the loss of the node that ran it.
