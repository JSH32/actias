...
`conn:send(value)` writes one JSON frame. Apart from the envelope a
class asks for with `event = "forward"`, there is no platform wire
...
the last handler left. It is a memory economy, not durability; a node
dying still drops its sockets, and reconnect-with-catch-up is still
the recovery story.

What a hibernated connection pays depends on what it declared. An
undeclared handler costs nothing at all: a `close` with no `close`
...
the platform writes that frame itself. A connection that only relays
what it follows therefore stays hibernated for its whole life, which
is what makes many thousands of them per node ordinary rather than
notable.
