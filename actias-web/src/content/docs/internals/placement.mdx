...
ships the latest snapshot to blob storage, coalescing bursts; a
graceful stop flushes every dirty object before the process leaves.
The epoch fences these ships: a zombie ex-holder's upload loses to
any newer epoch.

A claim restores the last shipped snapshot whenever the store's copy
is newer than the node's own file: a sidecar records the epoch of the
node's last residency, so a node returning to an object it hosted
long ago cannot serve stale state. A failed ship is logged and
retried by the next write; local durability holds regardless.
