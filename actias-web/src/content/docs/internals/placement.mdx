...
ships to blob storage, coalescing bursts; a graceful stop flushes
every dirty object before the process leaves. Small objects ship as
a whole snapshot. Past a size threshold the flight ships only the
write-ahead log frames committed since the last one, against a base
laid down at a checkpoint, so the cost of a ship follows the delta,
not the database; the log folds into a fresh base when it grows. The
epoch fences these ships either way: a zombie ex-holder's upload
loses to any newer epoch.
...
replayed by SQLite itself, or the snapshot directly. A sidecar
records the epoch of the node's last residency, so a node returning
to an object it hosted long ago cannot serve stale state. A failed
ship is logged and retried by the next write; local durability holds
regardless.
...
flowchart LR
    ...
    c -- no --> pull[Restore shipped state,<br/>then the replica file]
```
