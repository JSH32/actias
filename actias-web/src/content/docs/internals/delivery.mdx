...
The payload does not multiply either. A node's batch carries the due
events **once**, in publish order, beside a list naming each edge and
the slice it is owed: its topic, how far it has already heard, and its
filter. The receiving node does the slicing. So the 5,000 watchers
above move one copy of a message per node, not 5,000, and the
publisher reads its own log once per topic in the batch rather than
once per edge.
...
every edge that connection held.

From the inbox, what happens next depends on the program: a class
declaring an `event` handler runs it, in a vm built or woken for the
purpose; a class declaring `event = "forward"` has the platform write
the envelope itself, which costs a serialize and a send with no vm
involved. That is what lets a hibernated connection stay hibernated
while still receiving what it follows.
...
A recorded node that no longer exists is not a failure. Node ids
never return, so connection edges batched to one prune outright, and
a durable edge's stale home clears: delivery falls back to routing
the follower's identity, which finds wherever it rehomed. A follower
never loses its subscription to its node dying.

## Large payloads

Events ride inline in a durable edge's batch up to 32 KB. Past that,
the batch
...
numbers, not payloads, for heavy traffic. Connection batches have no
such range form and always ride inline, which they can afford because
a node's copy is one per event however many sockets are owed it.
